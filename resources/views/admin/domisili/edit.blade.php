...
<x-admin.bread-crumb
    ...
    :breadcrumbs="[
        ...
        ['label' => 'Ubah Domisili', 'route' => '']
    ]"
/>

<div class="bg-white dark:bg-neutral-800 p-4 rounded-md">
    <form action="{{ route('admin.domisili.update', $domisili->uuid) }}" method="post">
        @csrf
        @method('PUT')
        <div class="grid gap-y-4">
            <div class="grid grid-cols-2">
                <x-ui.select-form label="*Penduduk" name="penduduk_id" :option="$penduduk" :selected="$domisili->penduduk_id" />
                <x-ui.select-form label="*No. RT" name="rt_id" :option="$rt" :selected="$domisili->rt_id" />
            </div>
            <div>
                ...
                <div class="flex gap-x-6">
                    <x-ui.radio-form 
                        ...
                        value="tetap" 
                        checked="{{ $domisili->status_tempat_tinggal === 'tetap' }}" 
                    />
                    ...
                    <x-ui.radio-form 
                        ...
                        name="status_tempat_tinggal" 
                        value="sementara" 
                        checked="{{ $domisili->status_tempat_tinggal === 'sementara' }}" 
                    />
                </div>
            </div>
        </div>
